...
import { getPurchaseBarcodes } from "@/services/purchase_service";
...
export default function BarcodeDisplay({ purchaseId }: Props) {
  ...
  useEffect(() => {
    const fetchBarcodes = async () => {
      try {
        const data = await getPurchaseBarcodes(purchaseId);
        setBarcodes(data);
      } catch (error) {
  ...
  const handleDownload = () => {
    const csvContent = [
      ["Barcode", "Item Name", "Unit Price", "Status"],
      ...barcodes.map(b => [b.barcode, b.itemName, b.unitPrice, b.status])
    ...
    link.href = url;
    link.download = `barcodes_purchase_${purchaseId}.csv`;
